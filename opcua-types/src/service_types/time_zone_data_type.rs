...
impl opcua::types::BinaryEncodable for TimeZoneDataType {
    ...
    fn byte_len(&self, ctx: &opcua::types::Context<'_>) -> usize {
        let mut size = 0usize;
        size += self.offset.byte_len(ctx);
        size += self.daylight_saving_in_offset.byte_len(ctx);
        size
    }
    #[allow(unused_variables)]
    fn encode<S: std::io::Write + ?Sized>(
        ...
        let mut size = 0usize;
        size += self.offset.encode(stream, ctx)?;
        size += self.daylight_saving_in_offset.encode(stream, ctx)?;
        Ok(size)
    }
}
