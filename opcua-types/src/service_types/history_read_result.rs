// OPCUA for Rust
// SPDX-License-Identifier: MPL-2.0
// Copyright (C) 2017-2024 Adam Lock, Einar Omang
//
// This file was autogenerated from tools/schema/schemas/1.0.4/Opc.Ua.Types.bsd by opcua-codegen
//
// DO NOT EDIT THIS FILE
#[derive(Debug, Clone, PartialEq)]
#[derive(Default)]
pub struct HistoryReadResult {
    pub status_code: crate::status_code::StatusCode,
    pub continuation_point: crate::byte_string::ByteString,
    pub history_data: crate::extension_object::ExtensionObject,
}
impl crate::MessageInfo for HistoryReadResult {
    fn object_id(&self) -> crate::ObjectId {
        crate::ObjectId::HistoryReadResult_Encoding_DefaultBinary
    }
}
impl crate::BinaryEncoder for HistoryReadResult {
    fn byte_len(&self) -> usize {
        let mut size = 0usize;
        size += self.status_code.byte_len();
        size += self.continuation_point.byte_len();
        size += self.history_data.byte_len();
        size
    }
    #[allow(unused_variables)]
    fn encode<S: std::io::Write>(&self, stream: &mut S) -> crate::EncodingResult<usize> {
        let mut size = 0usize;
        size += self.status_code.encode(stream)?;
        size += self.continuation_point.encode(stream)?;
        size += self.history_data.encode(stream)?;
        Ok(size)
    }
    #[allow(unused_variables)]
    fn decode<S: std::io::Read>(
        stream: &mut S,
        decoding_options: &crate::DecodingOptions,
    ) -> crate::EncodingResult<Self> {
        let status_code = <crate::status_code::StatusCode as crate::BinaryEncoder>::decode(
            stream,
            decoding_options,
        )?;
        let continuation_point = <crate::byte_string::ByteString as crate::BinaryEncoder>::decode(
            stream,
            decoding_options,
        )?;
        let history_data = <crate::extension_object::ExtensionObject as crate::BinaryEncoder>::decode(
            stream,
            decoding_options,
        )?;
        Ok(Self {
            status_code,
            continuation_point,
            history_data,
        })
    }
}
