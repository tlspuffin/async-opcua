// OPCUA for Rust
// SPDX-License-Identifier: MPL-2.0
// Copyright (C) 2017-2024 Adam Lock, Einar Omang
//
// This file was autogenerated from tools/schema/schemas/1.0.4/Opc.Ua.Types.bsd by opcua-codegen
//
// DO NOT EDIT THIS FILE
#[derive(Debug, Clone, PartialEq)]
#[derive(Default)]
pub struct RolePermissionType {
    pub role_id: crate::node_id::NodeId,
    pub permissions: super::enums::PermissionType,
}
impl crate::MessageInfo for RolePermissionType {
    fn object_id(&self) -> crate::ObjectId {
        crate::ObjectId::RolePermissionType_Encoding_DefaultBinary
    }
}
impl crate::BinaryEncoder for RolePermissionType {
    fn byte_len(&self) -> usize {
        let mut size = 0usize;
        size += self.role_id.byte_len();
        size += self.permissions.byte_len();
        size
    }
    #[allow(unused_variables)]
    fn encode<S: std::io::Write>(&self, stream: &mut S) -> crate::EncodingResult<usize> {
        let mut size = 0usize;
        size += self.role_id.encode(stream)?;
        size += self.permissions.encode(stream)?;
        Ok(size)
    }
    #[allow(unused_variables)]
    fn decode<S: std::io::Read>(
        stream: &mut S,
        decoding_options: &crate::DecodingOptions,
    ) -> crate::EncodingResult<Self> {
        let role_id = <crate::node_id::NodeId as crate::BinaryEncoder>::decode(
            stream,
            decoding_options,
        )?;
        let permissions = <super::enums::PermissionType as crate::BinaryEncoder>::decode(
            stream,
            decoding_options,
        )?;
        Ok(Self { role_id, permissions })
    }
}
