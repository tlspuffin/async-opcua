...
impl opcua::types::BinaryEncodable for ThreeDFrame {
    ...
    fn byte_len(&self, ctx: &opcua::types::Context<'_>) -> usize {
        let mut size = 0usize;
        size += self.cartesian_coordinates.byte_len(ctx);
        size += self.orientation.byte_len(ctx);
        size
    }
    #[allow(unused_variables)]
    fn encode<S: std::io::Write + ?Sized>(
        ...
        let mut size = 0usize;
        size += self.cartesian_coordinates.encode(stream, ctx)?;
        size += self.orientation.encode(stream, ctx)?;
        Ok(size)
    }
}
