// This file was autogenerated from schemas/1.0.4/Opc.Ua.Types.bsd by opcua-codegen
//
// DO NOT EDIT THIS FILE

// OPCUA for Rust
// SPDX-License-Identifier: MPL-2.0
// Copyright (C) 2017-2024 Adam Lock, Einar Omang
#[allow(unused)]
mod opcua { pub use crate as types; }
#[derive(Debug, Clone, PartialEq)]
#[cfg_attr(
    feature = "json",
    derive(opcua::types::JsonEncodable, opcua::types::JsonDecodable)
)]
#[cfg_attr(feature = "xml", derive(opcua::types::FromXml))]
#[derive(Default)]
pub struct TrustListDataType {
    pub specified_lists: u32,
    pub trusted_certificates: Option<Vec<opcua::types::byte_string::ByteString>>,
    pub trusted_crls: Option<Vec<opcua::types::byte_string::ByteString>>,
    pub issuer_certificates: Option<Vec<opcua::types::byte_string::ByteString>>,
    pub issuer_crls: Option<Vec<opcua::types::byte_string::ByteString>>,
}
impl opcua::types::MessageInfo for TrustListDataType {
    fn type_id(&self) -> opcua::types::ObjectId {
        opcua::types::ObjectId::TrustListDataType_Encoding_DefaultBinary
    }
    fn json_type_id(&self) -> opcua::types::ObjectId {
        opcua::types::ObjectId::TrustListDataType_Encoding_DefaultJson
    }
    fn xml_type_id(&self) -> opcua::types::ObjectId {
        opcua::types::ObjectId::TrustListDataType_Encoding_DefaultXml
    }
}
impl opcua::types::BinaryEncodable for TrustListDataType {
    #[allow(unused_variables)]
    fn byte_len(&self, ctx: &opcua::types::Context<'_>) -> usize {
        let mut size = 0usize;
        size += self.specified_lists.byte_len(ctx);
        size += self.trusted_certificates.byte_len(ctx);
        size += self.trusted_crls.byte_len(ctx);
        size += self.issuer_certificates.byte_len(ctx);
        size += self.issuer_crls.byte_len(ctx);
        size
    }
    #[allow(unused_variables)]
    fn encode<S: std::io::Write + ?Sized>(
        &self,
        stream: &mut S,
        ctx: &opcua::types::Context<'_>,
    ) -> opcua::types::EncodingResult<usize> {
        let mut size = 0usize;
        size += self.specified_lists.encode(stream, ctx)?;
        size += self.trusted_certificates.encode(stream, ctx)?;
        size += self.trusted_crls.encode(stream, ctx)?;
        size += self.issuer_certificates.encode(stream, ctx)?;
        size += self.issuer_crls.encode(stream, ctx)?;
        Ok(size)
    }
}
impl opcua::types::BinaryDecodable for TrustListDataType {
    #[allow(unused_variables)]
    fn decode<S: std::io::Read + ?Sized>(
        stream: &mut S,
        ctx: &opcua::types::Context<'_>,
    ) -> opcua::types::EncodingResult<Self> {
        Ok(Self {
            specified_lists: opcua::types::BinaryDecodable::decode(stream, ctx)?,
            trusted_certificates: opcua::types::BinaryDecodable::decode(stream, ctx)?,
            trusted_crls: opcua::types::BinaryDecodable::decode(stream, ctx)?,
            issuer_certificates: opcua::types::BinaryDecodable::decode(stream, ctx)?,
            issuer_crls: opcua::types::BinaryDecodable::decode(stream, ctx)?,
        })
    }
}
