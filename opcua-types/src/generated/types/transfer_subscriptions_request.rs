...
impl opcua::types::BinaryEncodable for TransferSubscriptionsRequest {
    ...
    fn byte_len(&self, ctx: &opcua::types::Context<'_>) -> usize {
        ...
        size += self.subscription_ids.byte_len(ctx);
        size += self.send_initial_values.byte_len(ctx);
        size
    }
    #[allow(unused_variables)]
    fn encode<S: std::io::Write + ?Sized>(
        ...
        size += self.subscription_ids.encode(stream, ctx)?;
        size += self.send_initial_values.encode(stream, ctx)?;
        Ok(size)
    }
}
impl opcua::types::BinaryDecodable for TransferSubscriptionsRequest {
    ...
    fn decode<S: std::io::Read + ?Sized>(
        ...
        Ok(Self {
            ...
            send_initial_values: opcua::types::BinaryDecodable::decode(stream, ctx)
                .map_err(|e| e.with_request_handle(__request_handle))?,
        })
    }
}
