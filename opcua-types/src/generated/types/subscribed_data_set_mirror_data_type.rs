...
impl opcua::types::BinaryEncodable for SubscribedDataSetMirrorDataType {
    ...
    fn byte_len(&self, ctx: &opcua::types::Context<'_>) -> usize {
        let mut size = 0usize;
        size += self.parent_node_name.byte_len(ctx);
        size += self.role_permissions.byte_len(ctx);
        size
    }
    #[allow(unused_variables)]
    fn encode<S: std::io::Write + ?Sized>(
        ...
        let mut size = 0usize;
        size += self.parent_node_name.encode(stream, ctx)?;
        size += self.role_permissions.encode(stream, ctx)?;
        Ok(size)
    }
}
