...
impl opcua::types::BinaryEncodable for BrowsePathTarget {
    ...
    fn byte_len(&self, ctx: &opcua::types::Context<'_>) -> usize {
        let mut size = 0usize;
        size += self.target_id.byte_len(ctx);
        size += self.remaining_path_index.byte_len(ctx);
        size
    }
    #[allow(unused_variables)]
    fn encode<S: std::io::Write + ?Sized>(
        ...
        let mut size = 0usize;
        size += self.target_id.encode(stream, ctx)?;
        size += self.remaining_path_index.encode(stream, ctx)?;
        Ok(size)
    }
}
