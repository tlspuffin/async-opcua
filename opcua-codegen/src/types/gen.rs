...
impl CodeGenerator {
    ...
    fn generate_bitfield(&self, item: EnumType) -> Result<GeneratedItem, CodeGenError> {
        ...
        let opcua_path: Path = syn::parse_str(&self.config.opcua_crate_path)?;
        ...
        impls.push(parse_quote! {
            impl #opcua_path::types::BinaryEncoder<#enum_ident> for #enum_ident {
                ...
                fn encode<S: std::io::Write>(&self, stream: &mut S) -> #opcua_path::types::EncodingResult<usize> {
                    #opcua_path::types::#write_method(stream, self.bits())
                }
    ...
    fn generate_struct(&self, item: StructuredType) -> Result<GeneratedItem, CodeGenError> {
            ...
            .is_some_and(|t| t == "ua:ExtensionObject")
        {
            ...
            impls.push(parse_quote! {
                impl #opcua_path::types::MessageInfo for #struct_ident {
                    fn object_id(&self) -> #opcua_path::types::ObjectId {
                        #opcua_path::types::ObjectId::#encoding_ident
                    }
                }
            });
        }
