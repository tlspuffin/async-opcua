// OPCUA for Rust
// SPDX-License-Identifier: MPL-2.0
// Copyright (C) 2017-2024 Adam Lock, Einar Omang
//
// This file was autogenerated from tools/schema/schemas/1.0.4/Opc.Ua.Types.bsd by opcua-codegen
//
// DO NOT EDIT THIS FILE
#[derive(Debug, Clone, PartialEq, Default)]
pub struct NodeReference {
    pub node_id: crate::types::node_id::NodeId,
    pub reference_type_id: crate::types::node_id::NodeId,
    pub is_forward: bool,
    pub referenced_node_ids: Option<Vec<crate::types::node_id::NodeId>>,
}
impl crate::types::MessageInfo for NodeReference {
    fn object_id(&self) -> crate::types::ObjectId {
        crate::types::ObjectId::NodeReference_Encoding_DefaultBinary
    }
}
impl crate::types::BinaryEncoder<NodeReference> for NodeReference {
    fn byte_len(&self) -> usize {
        let mut size = 0usize;
        size += self.node_id.byte_len();
        size += self.reference_type_id.byte_len();
        size += self.is_forward.byte_len();
        size += self.referenced_node_ids.byte_len();
        size
    }
    #[allow(unused_variables)]
    fn encode<S: std::io::Write>(&self, stream: &mut S) -> crate::types::EncodingResult<usize> {
        let mut size = 0usize;
        size += self.node_id.encode(stream)?;
        size += self.reference_type_id.encode(stream)?;
        size += self.is_forward.encode(stream)?;
        size += self.referenced_node_ids.encode(stream)?;
        Ok(size)
    }
    #[allow(unused_variables)]
    fn decode<S: std::io::Read>(
        stream: &mut S,
        decoding_options: &crate::types::DecodingOptions,
    ) -> crate::types::EncodingResult<Self> {
        let node_id = <crate::types::node_id::NodeId as crate::types::BinaryEncoder<
            crate::types::node_id::NodeId,
        >>::decode(stream, decoding_options)?;
        let reference_type_id = <crate::types::node_id::NodeId as crate::types::BinaryEncoder<
            crate::types::node_id::NodeId,
        >>::decode(stream, decoding_options)?;
        let is_forward =
            <bool as crate::types::BinaryEncoder<bool>>::decode(stream, decoding_options)?;
        let referenced_node_ids =
            <Option<Vec<crate::types::node_id::NodeId>> as crate::types::BinaryEncoder<
                Option<Vec<crate::types::node_id::NodeId>>,
            >>::decode(stream, decoding_options)?;
        Ok(Self {
            node_id,
            reference_type_id,
            is_forward,
            referenced_node_ids,
        })
    }
}
